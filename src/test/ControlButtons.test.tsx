...
describe("ControlButtons", () => {
    ...
    it("通常時は評価・クリア・わからないボタンが表示される", () => {
        ...
        expect(screen.getByRole("button", { name: "わからない" }))
            .toBeDefined();
    ...
    it("showNext が true の時は次の問題へボタンが表示される", () => {
        ...
        expect(screen.getByRole("button", { name: "つぎの もんだいへ" }))
            .toBeDefined();
    });
    ...
    it("ストロークがない時は評価・クリアボタンが無効になる", () => {
        ...
        expect(screen.getByRole("button", { name: "わからない" })).not
            .toBeDisabled();
    });

    it("さいしょからボタンは常に表示される", () => {
        const { rerender } = render(<ControlButtons {...defaultProps} />);
        expect(screen.getByRole("button", { name: "さいしょから" }))
            .toBeDefined();
