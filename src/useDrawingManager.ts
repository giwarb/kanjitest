...
export const useDrawingManager = (canvas: HTMLCanvasElement | null) => {
    ...
    useEffect(() => {
        ...
        const getCanvasCoord = (evt: MouseEvent | TouchEvent): { x: number, y: number } => {
            const rect = canvas.getBoundingClientRect();
            return {
                x: (evt instanceof MouseEvent ? evt.clientX : evt.touches[0].clientX) - (rect?.left || 0),
                y: (evt instanceof MouseEvent ? evt.clientY : evt.touches[0].clientY) - (rect?.top || 0)
            };
        };
